  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
        <button onClick={() => changeMonth(-1)} style={{ width:36, height:36, borderRadius:"50%", border:"1px solid #ddd", background:"rgba(255,255,255,0.6)", fontSize:18, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>‹</button>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:4 }}>
        {cells.map((cell, i) => {
          ...
          return (
            <div key={i} onClick={() => cell.current && onDayClick(dateKey(curYear, curMonth, cell.day))}
              style={{ background: cell.current ? "rgba(255,255,255,0.65)" : "rgba(255,255,255,0.2)", minHeight:82, padding:"6px 5px", cursor: cell.current ? "pointer" : "default", borderRadius:10, border: isToday ? "2px solid #7F77DD" : "1px solid rgba(0,0,0,0.06)", transition:"background 0.15s, transform 0.1s" }}
            ...
            >
              <div style={{ width:24, height:24, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:"50%", background: isToday ? "#7F77DD" : "transparent", color: isToday ? "#fff" : cell.current ? "#333" : "#ccc", fontSize:12, fontWeight: isToday ? 700 : 500, marginBottom:3 }}>
                {cell.day}
              </div>
