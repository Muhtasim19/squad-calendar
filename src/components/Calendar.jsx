...
export default function Calendar({ events, categories, onDayClick }) {
  ...
  function getCatColor(type) {
    ...
    return cat ? cat.color : (DEFAULT_COLORS[type] || { bg:"#E0E0E0", color:"#555" });
  }
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
        <button onClick={() => changeMonth(-1)} style={{ width:36, height:36, borderRadius:"50%", border:"1px solid #ddd", background:"rgba(255,255,255,0.6)", fontSize:18, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(8px)" }}>‹</button>
        ...
        {cells.map((cell, i) => {
          ...
          return (
            ...
            >
              ...
              {dayEvents.slice(0,2).map(ev => <EventPill key={ev.id} event={ev} color={getCatColor(ev.type)} />)}
